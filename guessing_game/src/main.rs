...
fn main() {
    ...
    //println!("The secret number is: {secret_number}");// Hide when done
    ...
    loop {
        ...
        let guess: u32 = match guess.trim().parse() {
            Ok(num) => num,// returns the converted number to guess variable
            Err(_) => continue,// ends current iteration of the loop, but does not break it
        };
        ...
        match guess.cmp(&secret_number) {
            ...
            Ordering::Equal => {// Code blocks can be used
                println!("You win!");
                break;// breaks the loop to end game on correct guess.
            }
        }
    }
}
